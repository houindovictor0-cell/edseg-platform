...
<section class="max-w-7xl mx-auto px-6 py-16">
    ...
    <a href="{{ route('actualites.index', ['categorie' => $filtre != 'Tous' ? $filtre : null]) }}"
       ...
       {{ request('categorie') == $filtre || ($filtre == 'Tous' && !request('categorie'))
            ...
            : 'border-gray-300 text-[#1A1A1A] hover:border-[#0B6E33] hover:text-[#0B6E33]' }}">
        {{ $filtre }}
    </a>
...
<img
    ...
    class="w-full h-44 object-cover mb-4">
            ...
            @endif
            <span class="text-[10px] font-bold uppercase tracking-widest text-[#C99000] mb-2 block">
                {{ $actu->categorie }}
            </span>
            <h4 class="font-semibold text-gray-900 text-base leading-snug mb-3 group-hover:text-[#0B6E33] transition">
                {{ $actu->titre }}
            </h4>
            <p class="text-[#1A1A1A] text-sm mb-4 leading-relaxed">{{ Str::limit($actu->contenu, 100) }}</p>
            <div class="flex justify-between items-center">
                <span class="text-xs text-[#CE1126]">{{ $actu->date_publication?->format('d M Y') }}</span>
                <a href="{{ route('actualites.show', $actu->id) }}"
                   class="text-xs font-semibold text-[#0B6E33] hover:underline">Lire →</a>
            </div>
        </article>
        @endforeach
    </div>
    ...
    @else
    <div class="text-center py-16 text-[#CE1126]">
